...
def create_education(*, account: Account, school_name: str, start_date: date, end_date: date, major: str, degree: str) -> list:
    ...
    e.save()
    return list_education(account.id)


def update_education(*, account: Account, id: int, school_name: str, start_date: date, end_date: date, major: str, degree: str) -> list:
    e = Education.objects.filter(id=id).first()
    if e is None:
        raise InvalidInputFormat("Old education entry not found!")
    e.school = get_school_with_name(school_name)
