...
def get_skill_tag(*, query: str) -> dict:
    ...
    tmp = Skill.objects.filter(Q(name__icontains=query) & ~Q(name__istartswith=query))
...
def get_school_tag(*, query: str) -> list:
    ...
    temp = School.objects.filter(Q(name__icontains=query) & ~Q(name__istartswith=query))
...
def get_company_tag(*, query: str) -> dict:
    ...
    tag_startwith_query = [{'name': t.name, 'logo': t.profile_picture} for t in temp]
    temp = Company.objects.filter(Q(name__icontains=query) & ~Q(name__istartswith=query))
